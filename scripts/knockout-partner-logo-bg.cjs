/**
 * Remove edge-connected canvas only (keeps logo text).
 * - Dark flood: RGB <= DARK (default 24) — true black bg goes; typical grey text (#404040+) stays.
 * - Light flood: RGB >= LIGHT (default 249) — white canvas only, not light grey glyphs.
 * No hole-punch / dilate (those erased small text and counters).
 */
const sharp = require('sharp');
const fs = require('fs');
const path = require('path');

const DIR = path.join(__dirname, '../public/book-to-video/partner-logos');
const DARK = 24;
const LIGHT = 249;
const MIN_ALPHA = 96;
...
/**
 * Thin black between coloured glyphs / transparent gaps (bad anti-alias on exports).
 * Only dark pixels with enough bright or transparent 8-neighbours become transparent.
 */
  ...
  for (let iter = 0; iter < iters; iter++) {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        if (maxCh(out, w, x, y) > DARK) continue;
        let support = 0;
        for (let k = 0; k < 8; k++) {
          const nx = x + DX[k];
          const ny = y + DY[k];
...
const WORDMARK_FRINGE_FILES = new Set(['facebook.png', 'trustpilot.png']);
/** Enclosed pure-black letter counters (p, o, e, b, R, etc.) */
const LETTER_HOLE_FILES = new Set(['bark.png', 'facebook.png', 'trustpilot.png']);
...
/** Remove enclosed pure-black letter counters (bad export); not edge-connected. */
...
async function main() {
  ...
  for (const f of files) {
    ...
    await processPng(fp);
    if (WORDMARK_FRINGE_FILES.has(f)) await processWordmarkFringe(fp);
    if (LETTER_HOLE_FILES.has(f)) await processLetterHoles(fp);
    console.log('knocked out bg:', f);
  }
}
